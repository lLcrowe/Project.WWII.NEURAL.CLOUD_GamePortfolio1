...
namespace lLCroweTool.QC.EditorOnly
{
    public static class lLcroweUtilEditor
    {
        ...
        public static void CreateDataObject<T>(ref T newDataObject, string fileName, string folderName, string tag, bool isUseDisplay = true) where T : ScriptableObject
        {
            if (fileName == "")
            {
        ...
        public static void UnitObjectInfoShow(ref UnitObjectInfo unitObjectInfo, ref SkillObjectScript skillObjectScript, ref UnitStatusValue newUnitStatusValue, ref UnitStateType newUnitStateType)
        {
            ...
            ObjectFieldAndNullButton("스킬정보",ref skillObjectScript);
        ...
        public static void ObjectFieldAndNullButton<T>(string contentText, ref T targetObject) where T : Object
        {
            EditorGUILayout.BeginHorizontal();            
            targetObject = EditorGUILayout.ObjectField(contentText, targetObject, typeof(T), true) as T;
            if (GUILayout.Button("Emtpy"))
            {
                targetObject = null;
            }
            EditorGUILayout.EndHorizontal();
        ...
        /// <summary>
        /// OnGUI의 GUI개체의 백그라운드 컬러를 변경하는 함수의 마지막구역
        /// </summary>
        ...
        public static T[] GetChildComponent<T>(Component targetTr, bool includeInactive = false) where T : Component
        {
        ...
        public static void CheckEmptyComponentForNewGameObject<T>(Transform tr, string findGameObjectName, ref T targetComponent, Transform parentTr = null) where T : Component
        {
            ...
            {
                tempComponent = tempTr.AddComponent<T>();
            }
        ...
        {
            return GetUnityObjectFile<T>(path, extend);
        ...
        public static T[] GetUnityObjectFile<T>(string path, string extend) where T : Object
        {
            //경로 : "Assets/A0/GameData/ItemDataFolder";
            //확장자 : "*.asset"//"*.Prefab"
            FileInfo[] fileInfoArray = GetFiles(path, extend);
        ...
        /// <summary>
        ///데이터나오는게 List<Dic<string, object>> => 줄(Line)<데이터<헤더(Header), 값(Value)>> => 1번줄 <ItemID, 돌멩이>
        ///키 => 라인(줄)
